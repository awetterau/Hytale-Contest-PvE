...
public final class GameDoorBlockDetectionSystem extends RefSystem<ChunkStore> {
    ...
    private void syncRegistry(@Nonnull Ref<ChunkStore> ref, @Nonnull CommandBuffer<ChunkStore> commandBuffer, boolean adding) {
        ...
        if (!adding) {
            GameDoorTriggerRegistry.unregister(worldId, pos);
            return;
        }

        if (!isDoorBaseBlock(world, pos)) {
            return;
        }

        GameDoorTriggerRegistry.register(worldId, pos);
    }

    private static boolean isDoorBaseBlock(@Nonnull World world, @Nonnull Vector3i pos) {
        BlockType blockType = world.getBlockType(pos);
        if (blockType == null || !GameDoorInteractionHandler.GAME_DOOR_BLOCK_ID.equalsIgnoreCase(blockType.getId())) {
            return false;
        }

        Vector3i below = new Vector3i(pos.x, pos.y - 1, pos.z);
        BlockType belowType = world.getBlockType(below);
        return belowType == null || !GameDoorInteractionHandler.GAME_DOOR_BLOCK_ID.equalsIgnoreCase(belowType.getId());
    }
}
