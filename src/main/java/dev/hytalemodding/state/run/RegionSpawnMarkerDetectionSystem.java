...
public final class RegionSpawnMarkerDetectionSystem extends RefSystem<ChunkStore> {
    ...
    private static void syncRegistry(@Nonnull Ref<ChunkStore> ref, @Nonnull CommandBuffer<ChunkStore> commandBuffer, boolean adding) {
        ...
        if (!adding) {
            RegionSpawnMarkerRegistry.unregister(worldId, pos);
            return;
        }
        ...
        String regionId = RegionSpawnMarkerRegistry.findRegionForBlockId(type == null ? null : type.getId());
        if (regionId == null) {
            return;
        }
        RegionSpawnMarkerRegistry.register(worldId, regionId, pos);
    }
}
